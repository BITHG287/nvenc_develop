...
def RGB2YUV_bt2020(R, G, B):
    ...
    Cb = (224 * Cb + 128) * 4
    Cr = (224 * Cr + 128) * 4
    Y = cp.clip(Y, 0, 1023).astype(cp.uint16)
    Cb = cp.clip(Cb, 0, 1023).astype(cp.uint16)
    Cr = cp.clip(Cr, 0, 1023).astype(cp.uint16)
...
def linear_rgb_to_yuv_hlg(img):
    ...
    upper = (img > 1/12) * (a * cp.log(img_b) + c)

    RGB = lower + upper
    hlg_R, hlg_G, hlg_B = RGB[:, :, 0], RGB[:, :, 1], RGB[:, :, 2]
    Y, Cb, Cr = RGB2YUV_bt2020(hlg_R, hlg_G, hlg_B)
    return cp.asnumpy(Y), cp.asnumpy(Cb), cp.asnumpy(Cr)
...
def img888_to_yuvp10le(img, gain=300):    
    ...
    Y, U, V = linear_rgb_to_yuv_hlg(linear_2020_gpu)
    return Y, U, V
